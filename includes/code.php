...
/*Add Channel*/
if (!empty($_POST['action']) && $_POST['action'] == "add_new_user") {
	// Check if email already exists
	$email = $_REQUEST['email'];
	$user_id = $_REQUEST['new_user_id'] ?? null;

	// Check if the email is already associated with the user being updated
	$email_check_query = "SELECT * FROM users WHERE email = '$email'";
	$email_check_result = mysqli_query($dbc, $email_check_query);

	if (mysqli_num_rows($email_check_result) > 0) {
		$row = mysqli_fetch_assoc($email_check_result);
		// Allow update if the email belongs to the user being updated
		if (!empty($user_id) && $row['user_id'] == $user_id) {
			// Email belongs to the current user, so proceed with update
		} else {
	...
	if (empty($user_id)) {
		...
		if (update_data($dbc, "users", $data_user, 'user_id', $user_id)) {
