            ...
            if ($r > 9) {
              ...
              <li class="nav-item dropdown more">
                ...
                <ul class="dropdown-menu" aria-labelledby="moreDropdown">
                  <?php
                  $getNav2 = mysqli_query($dbc, "SELECT * FROM menus WHERE parent_id=0 AND page!='dashboard.php' ORDER BY sort_order ASC LIMIT 10 OFFSET 10");
              ...
              ?>
                <li class="nav-item dropdown">
                  ...
                  <?php if (countWhen($dbc, "menus", 'parent_id', $fetch_nav['id']) > 0) { ?>
                    <div class="dropdown-menu" aria-labelledby="ui-elementsDropdown">
                      <?php
                      $getChild = mysqli_query(
                        $dbc,
                        "SELECT * FROM menus 
                                WHERE parent_id='{$fetch_nav['id']}' 
                                AND page!='dashboard.php' 
                                AND id NOT IN ('122', '144', '111', '132', '143', '133', '140', '147', '114', '146', '145', '134', '135', '131', '129', '123', '125', '127')"
                      );
                      while ($child = mysqli_fetch_assoc($getChild)) {
                      ?>
                        <a class="nav-link pl-lg-2" href="<?= $child['page'] ?>">
