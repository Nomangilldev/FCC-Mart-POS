              ...
              <header class="d-none d-print-block">
                <div class="row">
                  ...
                  <div class="col-sm-5 mt-3">
                    <h1 style="margin-left: -20px; color: red;font-weight: bold;font-size: 30px"><?= $get_company['name'] ?>
                    </h1>
                            ...
                            if ($row['transaction_type'] == "credit_sale") {
                              ...
                              if ($check_remaing_balance < 0) {
                                ...
                                if ($next_date != 'special') {
                                  ...
                                  if ($total_days > 0) {
                                    $comment = '<span class="text-warning">' . $total_days . ' days left </span>(' . $next_date . ')';
                                  } else {

                                    $comment = '<span class="text-danger">' . abs($total_days) . ' ago Expired </span>';
                                  }
                                }
                            ...
                            if ($row['debit'] !== 0 and $row['credit'] !== 0):
                              ...
                              <tr>
                                ...
                                <?php if ($check_remaing_balance < 0 and $row['transaction_from'] == "invoice"): ?>
                                  <td class="text-danger"><?= number_format(((int) $row['credit'] - (int) $row['debit']) + (int) $temp) ?>
                                  </td>
                                <?php elseif ($row['transaction_from'] == "voucher"): ?>
                                  <td class="text-info"><?= number_format(((int) $row['credit'] - (int) $row['debit']) + (int) $temp) ?></td>
                                <?php else: ?>
                                  <td class="text-success"><?= number_format(((int) $row['credit'] - (int) $row['debit']) + (int) $temp) ?>
                                  </td>
                                <?php endif ?>
                                <?php if (isset($_POST['fullledger'])): ?>
