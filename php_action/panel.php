...
if (isset($_REQUEST['add_brand_name'])) {
    // Escape input
    $brand_name   = mysqli_real_escape_string($dbc, trim($_REQUEST['add_brand_name']));
    $brand_status = mysqli_real_escape_string($dbc, $_REQUEST['brand_status']);
    $brand_id     = isset($_REQUEST['brand_id']) ? intval($_REQUEST['brand_id']) : 0;

    // Check if brand already exists (ignore current brand_id when updating)
    if ($brand_id == 0) {
        $check_sql = "SELECT COUNT(*) as cnt FROM brands WHERE brand_name = '$brand_name'";
    } else {
        $check_sql = "SELECT COUNT(*) as cnt FROM brands 
    ...
    if ($row['cnt'] > 0) {
        ...
        // Update
            ...
            if (update_data($dbc, "brands", $data_array, "brand_id", $brand_id)) {
                $response = [
...
if (isset($_REQUEST['add_category_name'])) {
	$category_name = trim($_REQUEST['add_category_name']);
	$category_status = $_REQUEST['categories_status'];
	$category_id = $_REQUEST['categories_id'];

	// Check if category already exists (ignore current ID when updating)
	if ($category_id == '') {
		$check_sql = "SELECT COUNT(*) as cnt FROM categories WHERE categories_name = '$category_name'";
	} else {
		$check_sql = "SELECT COUNT(*) as cnt FROM categories 
	...
	if ($row['cnt'] > 0) {
		...
		$data_array = [
			...
			'categories_status' => $category_status,
		];
		...
		if ($category_id == '') {
			...
			if (update_data($dbc, "categories", $data_array, "categories_id", $category_id)) {
				$response = [
